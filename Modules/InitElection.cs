...
namespace ElectionBot.Modules
{
    public class InitElection : ModuleBase<SocketCommandContext>
    {
        ...
        {
            string update = $"UPDATE {(isAdmin ? "AdminVoters" : "ModVoters")} SET voter_key = @voter_key, weight = @weight WHERE guild_id = @guild_id AND user_id = @user_id;";
            string insert = $"INSERT INTO {(isAdmin ? "AdminVoters" : "ModVoters")} (guild_id, user_id, voter_key, weight) SELECT @guild_id, @user_id, @voter_key, @weight WHERE (SELECT Changes() = 0);";
